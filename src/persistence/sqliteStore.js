...
const sqliteStore = {
  ...
  _configureEncryption(passphraseRaw) {
    ...
    if (!encryptionMetaRaw) {
      if (!passphrase) {
        this._setMetaValue('encryption', JSON.stringify({ v: 1, enabled: false }));
    ...
    if (!encryptionMeta?.enabled) {
      this._encryption = { enabled: false, key: null };
      if (passphrase) {
        this._logger?.warn?.('WA2DC_DB_PASSPHRASE is set, but existing SQLite DB is not encrypted. Ignoring passphrase.');
      }
      return;
    }

    if (!passphrase) {
      throw new Error('SQLite DB is encrypted but WA2DC_DB_PASSPHRASE is not set.');
    }
  ...
  getAppState(key) {
    this._ensureDbReady();
    const row = this._db.prepare('SELECT value FROM app_state WHERE key = ?').get(key);
    if (!row) {
      return null;
    }
  ...
  getAuthCreds() {
    this._ensureDbReady();
    const row = this._db.prepare('SELECT value FROM auth_creds WHERE id = 1').get();
    if (!row) {
      return null;
    }
    return this._decodeStoredValue(row.value);
  ...
  getAuthKeys(fileKeys = []) {
    this._ensureDbReady();
    const result = {};
    const stmt = this._db.prepare('SELECT value FROM auth_keys WHERE file_key = ?');
